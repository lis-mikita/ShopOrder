...
{
    public class GetUserWithOrdersAsync
    {
        private readonly Mock<IUserRepository> _userRepositoryMock;
        private readonly IUserService _userService;
        private readonly GetUserWithOrdersAsync_TestData TestData;

        private User User { get; set; }

        public GetUserWithOrdersAsync()
        {
            _userRepositoryMock = new Mock<IUserRepository>();
            _userService = new UserService(_userRepositoryMock.Object);
            TestData = new GetUserWithOrdersAsync_TestData();
        ...
        {
            var actual = await _userService.GetUserWithOrdersAsync(id);
        ...
        [Fact]
        public async Task User_with_valid_id_should_return_UserResponse_with_User()
        {
            _userRepositoryMock.Setup(repo => repo.GetUserWithOrdersAsync(It.IsAny<int>()))
                .Returns(Task.FromResult<User?>(User));

            var actual = await _userService.GetUserWithOrdersAsync(User.UserId);
